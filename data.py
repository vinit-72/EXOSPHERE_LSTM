...
def load_data(data_tag:str, data_target:str, 
    ...
    # improvement for dataset balance, which is important for LSTM training
    ...
    for f,l,n,a in zip(seg_feature_vec, seg_label_vec, seg_num_vec, seg_attack_num_vec):
        total_packets = sum(n)
        total_attacks = sum(a)
        ...
        if total_attacks > 0:
            ...
            balanced_num.append(n)
            balanced_atk_num.append(a)
        else:
            if random.random() < 0.2:  # keep 20% of benign samples
                balanced_features.append(f)
                balanced_labels.append(l)
                balanced_num.append(n)
                balanced_atk_num.append(a)

    seg_feature_vec = balanced_features
    seg_label_vec = balanced_labels
    seg_num_vec = balanced_num
    seg_attack_num_vec = balanced_atk_num
