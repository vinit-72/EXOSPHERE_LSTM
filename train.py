...
def train_test(data_tag:str, log_path:str, fig_path:str,
    ...
    if use_lstm:
        exosphere = ExosphereLSTM(input_size=1, hidden_size=64, num_layers=3)
    else:
    ...
    for e in range(num_epoch):
        ...
        for th in [i * 0.5 for i in range(-20, 20)]:   # range: -10 → +10
            preds = [1 if sc > th else 0 for sc in true_pred]
            f1_temp = f1_score(true_label, preds, average='macro')

            if f1_temp > best_f1:
                best_f1 = f1_temp
                best_threshold = th

        waterline = best_threshold
